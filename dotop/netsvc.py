...
def LocalService(name):
    """
    The dotop.netsvc.LocalService() function is deprecated. It still works
    in two cases: workflows and reports. For workflows, instead of using
    LocalService('workflow'), dotop.workflow should be used (better yet,
    methods on dotop.osv.orm.Model should be used). For reports,
    dotop.report.render_report() should be used (methods on the Model should
    be provided too in the future).
    """
    assert dotop.conf.deprecation.allow_local_service
    _logger.warning("LocalService() is deprecated since march 2013 (it was called with '%s')." % name)

    if name == 'workflow':
        return dotop.workflow

    if name.startswith('report.'):
        report = dotop.report.interface.report_int._reports.get(name)
        if report:
            return report
        else:
...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('dotop.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
            cr.autocommit(True)
...
def init_logger():
    ...
    # Check that handler.stream has a fileno() method: when running dotop
...
DEFAULT_LOG_CONFIGURATION = [
    'dotop.workflow.workitem:WARNING',
    'dotop.http.rpc.request:INFO',
    'dotop.http.rpc.response:INFO',
    'dotop.addons.web.http:INFO',
    'dotop.sql_db:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['dotop:DEBUG','dotop.http.rpc.request:DEBUG', 'dotop.http.rpc.response:DEBUG'],
    'debug_rpc': ['dotop:DEBUG','dotop.http.rpc.request:DEBUG'],
    'debug': ['dotop:DEBUG'],
    'debug_sql': ['dotop.sql_db:DEBUG'],
    'info': [],
    'warn': ['dotop:WARNING', 'werkzeug:WARNING'],
    'error': ['dotop:ERROR', 'werkzeug:ERROR'],
    'critical': ['dotop:CRITICAL', 'werkzeug:CRITICAL'],
}
