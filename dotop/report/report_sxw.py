...
from dotop import fields
from dotop.exceptions import AccessError
from dotop.tools import DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT
...
class rml_parse(object):
    def __init__(self, cr, uid, name, parents=rml_parents, tag=rml_tag, context=None):
        ...
        self.cr = cr
        self.uid = uid
        env = dotop.api.Environment(cr, uid, context)
    ...
    def set_html_image(self, id, model=None, field=None, context=None):
        ...
        if not model:
            model = 'ir.attachment'
        try:
            env = dotop.api.Environment(self.cr, self.uid, {})
...
class report_sxw(report_rml, preprocess.report):
    ...
    dotop.netsvc.LocalService() indirection and the related
    dotop.report.interface.report_int._reports dictionary:
    ...
    def getObjects(self, cr, uid, ids, context):
        env = dotop.api.Environment(cr, uid, context or {})
