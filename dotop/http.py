...
import dotop
from dotop.service.server import memory_info
from dotop.service import security, model as service_model
from dotop.tools.func import lazy_property
from dotop.tools import ustr, consteq, frozendict

from dotop.modules.module import module_manifest
...
# *broken*
NO_POSTMORTEM = (dotop.osv.orm.except_orm,
                 dotop.exceptions.AccessError,
                 dotop.exceptions.ValidationError,
                 dotop.exceptions.MissingError,
                 dotop.exceptions.AccessDenied,
                 dotop.exceptions.Warning,
                 dotop.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                dotop.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = dotop.service.common.dispatch
        elif service_name == 'db':
            dispatch = dotop.service.db.dispatch
        elif service_name == 'object':
            dispatch = dotop.service.model.dispatch
        elif service_name == 'report':
            dispatch = dotop.service.report.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                dotop.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                dotop.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except dotop.exceptions.DeferredException, e:
        _logger.exception(dotop.tools.exception_to_unicode(e))
        dotop.tools.debugger.post_mortem(dotop.tools.config, e.traceback)
        raise
    except Exception, e:
        _logger.exception(dotop.tools.exception_to_unicode(e))
        dotop.tools.debugger.post_mortem(dotop.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all dotop Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in dotop.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~dotop.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~dotop.api.Environment` bound to current request. """
        if self._env is None:
            self._env = dotop.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    @lazy_property
    def session(self):
        """ :class:`dotopSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            dotop.tools.debugger.post_mortem(
                dotop.tools.config, sys.exc_info())
        raise
    ...
    def debug(self):
        ...
        """
        debug = 'devil' in self.httprequest.args
        if debug and self.httprequest.args.get('devil') == 'assets':
            debug = 'assets'
        ...
        if not debug and self.httprequest.referrer:
            debug = bool(urlparse.parse_qs(urlparse.urlparse(self.httprequest.referrer).query, keep_blank_values=True).get('devil'))
        return debug
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return dotop.registry(self.db) if self.db else None
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        dotop implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <dotop.http.WebRequest.csrf_token`, the
          :data:`~dotop.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          dotop) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (dotop.exceptions.Warning, SessionExpiredException, dotop.exceptions.except_orm)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "dotop Server Error",
                    'data': serialize_exception(exception)
            }
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "dotop Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = u"会话超时"
...
def serialize_exception(e):
    ...
    if isinstance(e, dotop.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, dotop.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, dotop.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, dotop.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, dotop.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, dotop.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, dotop.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, dotop.exceptions.except_orm):
...
dotop URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.dotop.com/documentation/9.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through dotop via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["dotop", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('dotop.addons.') and c.__module__.split(".")[2] in modules
...
class dotopSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(dotopSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def _fix_lang(self, context):
        """ dotop provides languages which may not make sense and/or may not
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or dotop.SUPERUSER_ID)
...
class DisableCacheMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        def start_wrapped(status, headers):
            ...
            debug = parsed.query.count('devil') >= 1
...
class Root(object):
    """Root WSGI application for the dotop Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = dotop.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(path, session_class=dotopSession)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + dotop.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in dotop.modules.module.ad_paths:
            for module in sorted(os.listdir(str(addons_path))):
                if module not in addons_module:
                    ...
                    if manifest_path and os.path.isdir(path_static):
                        ...
                        if 'dotop.addons' in sys.modules:
                            m = __import__('dotop.addons.' + module)
                        else:
                            m = None
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-dotop-Session-Id")
        if not sid:
    ...
    def get_response(self, httprequest, result, explicit_session):
        ...
        if not explicit_session and hasattr(response, 'set_cookie'):
            response.set_cookie('session_id', httprequest.session.sid, max_age=1 * 24 * 60 * 60)#90 * 24 * 60 * 60
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        dotop.registry(db).check_signaling()
                        with dotop.tools.mute_logger('dotop.sql_db'):
                        ...
                        if request.httprequest.path == '/web':
                            result = werkzeug.utils.redirect('/web/datapool/selector')
                        else:
...
def db_list(force=False, httprequest=None):
    dbs = dotop.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    r = dotop.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = dotop.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = dotop.tools.ustr(filename)
...
class CommonController(Controller):
    ...
    def jsonrpc(self, service, method, args):
        """ Method used by client APIs to contact dotop. """
