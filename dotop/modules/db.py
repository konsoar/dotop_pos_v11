...
def initialize(cr):
    ...
    """
    f = dotop.modules.get_module_resource('base', 'base.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)
    base_sql_file = dotop.tools.misc.file_open(f)
    try:
    ...
    for i in dotop.modules.get_modules():
        mod_path = dotop.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = dotop.modules.load_information_from_description_file(i)
...
def has_unaccent(cr):
    ...
    module but any similar function will be picked by dotop.
