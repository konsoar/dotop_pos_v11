...
import dotop
import dotop.tools as tools
import dotop.release as release
from dotop import SUPERUSER_ID, api

MANIFEST_NAMES = ['__dotop__.py']
...
class AddonsHook(object):
    """ Makes modules accessible through dotop.addons.* and dotop.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('dotop.addons.', 'dotoperp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        dotop_name = re.sub(r'^dotoperp.addons.(\w+)$', r'dotop.addons.\g<1>', name)
        dotoperp_name = re.sub(r'^dotop.addons.(\w+)$', r'dotoperp.addons.\g<1>', dotop_name)

        assert dotop_name not in sys.modules
        assert dotoperp_name not in sys.modules
        ...
        # create empty dotop.addons.* module, set name
        new_mod = types.ModuleType(dotop_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "dotop addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = dotop_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[dotop_name] = sys.modules[dotoperp_name] = new_mod
        ...
        # people import dotop.addons and expect dotop.addons.<module> to work
        setattr(dotop.addons, addon_name, new_mod)
...
class dotopHook(object):
    """ Makes dotop package also available as dotop """

    def find_module(self, name, path=None):
        # dotop.addons.<identifier> should already be matched by AddonsHook,
        # only framework and subdirectories of modules should match
        if re.match(r'^dotoperp\b', name):
            return self

    def load_module(self, name):
        assert name not in sys.modules

        canonical = re.sub(r'^dotoperp(.*)', r'dotop\g<1>', name)
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import dotop addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import dotop.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add dotop.addons.__path__
    for ad in __import__('dotop.addons').addons.__path__:
    ...
    if not hooked:
        ...
        sys.meta_path.append(dotopHook())
        hooked = True
...
def get_module_filetree(module, dir='.'):
    ...
    files = dotop.tools.osutil.listdir(path, True)
...
def load_dotop_module(module_name):
    """ Load an dotop module, if not already loaded.
    ...
    initialize_sys_path()
    try:
        __import__('dotop.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['dotop.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='dotop.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            t0 = time.time()
            t0_sql = dotop.sql_db.sql_counter
            _logger.info('%s running tests.', m.__name__)
            result = unittest.TextTestRunner(verbosity=2, stream=TestStream(m.__name__)).run(suite)
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, dotop.sql_db.sql_counter - t0_sql)
