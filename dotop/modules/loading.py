...
import dotop
import dotop.modules.db
import dotop.modules.graph
import dotop.modules.migration
import dotop.modules.registry
import dotop.tools as tools

from dotop import api, SUPERUSER_ID
from dotop.modules.module import adapt_version, initialize_sys_path, \
                                load_dotop_module, runs_post_install

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('dotop.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True, skip_modules=None, report=None):
    ...
    def load_test(module_name, idref, mode):
        ...
        try:
            ...
            if tools.config.options['test_commit']:
                ...
                dotop.registry(cr.dbname).clear_caches()
    ...
    processed_modules = []
    loaded_modules = []
    registry = dotop.registry(cr.dbname)
    migrations = dotop.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        load_dotop_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['dotop.addons.%s' % (module_name,)]
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            overwrite = dotop.tools.config["overwrite_existing_translations"]
            ...
            if has_demo:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(dotop.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, dotop.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    cr = db.cursor()
    try:
        if not dotop.modules.db.is_initialized(cr):
        ...
        # dotop.modules.registry.Registry.new().
        registry = dotop.registry(cr.dbname)
        env = api.Environment(cr, SUPERUSER_ID, {})
        ...
        graph = dotop.modules.graph.Graph()
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['dotop.addons.%s' % (pkg.name,)]
                ...
                return dotop.modules.registry.Registry.new(cr.dbname, force_demo, status, update_module)
        ...
        t0 = time.time()
        t0_sql = dotop.sql_db.sql_counter
        if dotop.tools.config['test_enable']:
            if update_module:
                cr.execute("SELECT name FROM ir_module_module WHERE state='installed' and name = ANY(%s)", (processed_modules,))
            else:
                cr.execute("SELECT name FROM ir_module_module WHERE state='installed'")
            for module_name in cr.fetchall():
                report.record_result(dotop.modules.module.run_unit_tests(module_name[0], cr.dbname, position=runs_post_install))
            _logger.log(25, "All post-tested in %.2fs, %s queries", time.time() - t0, dotop.sql_db.sql_counter - t0_sql)
    finally:
        cr.close()
