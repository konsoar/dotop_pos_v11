...
from dotop.workflow.helpers import Session
from dotop.workflow.helpers import Record
from dotop.workflow.helpers import WorkflowActivity
...
class WorkflowItem(object):
    ...
    def wkf_expr_execute_action(self, activity):
        ...
        env = dotop.api.Environment(self.session.cr, self.session.uid, context)
    ...
    def wkf_expr_check(self, transition, signal):
        ...
        if self.session.uid != dotop.SUPERUSER_ID and transition['group_id']:
            env = dotop.api.Environment(self.session.cr, self.session.uid, {})
