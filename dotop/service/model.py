...
import dotop
from dotop.exceptions import UserError, ValidationError, QWebException
from dotop.models import check_method_name
from dotop.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = dotop.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = dotop.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if dotop.registry(dbname)._init and not dotop.tools.config['test_enable']:
                    raise dotop.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    recs = dotop.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return dotop.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with dotop.registry(db).cursor() as cr:
