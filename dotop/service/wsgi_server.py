...
# User code must use the exceptions defined in ``dotop.exceptions`` (not
...
def xmlrpc_return(start_response, service, method, params, string_faultcode=False):
    ...
    """
    # Map dotop core exceptions to XML-RPC fault codes. Specific exceptions
    # defined in ``dotop.exceptions`` are mapped to specific fault codes;
    ...
    # exception handling.
    try:
        result = dotop.http.dispatch_rpc(service, method, params)
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, dotop.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, dotop.tools.ustr(e.value))
        ...
        response = xmlrpclib.dumps(fault, allow_none=False, encoding=None)
    elif isinstance(e, dotop.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = dotop.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, dotop.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(dotop.tools.ustr(e.message), formatted_info)
        response = xmlrpclib.dumps(fault, allow_none=False, encoding=None)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(dotop.tools.exception_to_unicode(e), formatted_info)
        response = xmlrpclib.dumps(fault, allow_none=None, encoding=None)
    return response
...
def application_unproxied(environ, start_response):
    ...
    # web.session.dotopSession.send() and at RPC dispatch in
    # dotop.service.web_services.objects_proxy.dispatch().
    ...
    with dotop.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, dotop.http.root]:
