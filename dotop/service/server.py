...
import dotop
from dotop.modules.registry import Registry
from dotop.release import nt_service_name
import dotop.tools.config as config
from dotop.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'dotop.service.http.request.%s' % (me.ident,)
...
class FSWatcher(object):
    def __init__(self):
        ...
        for path in dotop.modules.module.ad_paths:
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        while True:
            ...
            registries = dotop.modules.registry.Registry.registries
            _logger.debug('cron%d polling for jobs', number)
            for db_name, registry in registries.iteritems():
                while registry.ready:
                    try:
                        acquired = dotop.addons.base.ir.ir_cron.ir_cron._acquire_job(db_name)
                        if not acquired:
                            break
                    except Exception:
    ...
    def cron_spawn(self):
        ...
        for i in range(dotop.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="dotop.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="dotop.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        _logger.debug('--')
        dotop.modules.registry.Registry.delete_all()
        logging.shutdown()
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('dotop: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(dotop.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = dotop.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('dotop.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            import dotop.addons.base as base
...
def load_server_wide_modules():
    for m in dotop.conf.server_wide_modules:
        try:
            dotop.modules.module.load_dotop_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `dotop-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute dotop-server process with (nearly) the same arguments"""
    if dotop.tools.osutil.is_running_as_nt_service():
...
def load_test_file_yml(registry, test_file):
    with registry.cursor() as cr:
        dotop.tools.convert_yaml_import(cr, 'base', file(test_file), 'test', {}, 'init')
    ...
    for mod_name, mod_mod in sys.modules.items():
        if mod_mod:
            ...
            if test_path == mod_path:
                ...
                stream = dotop.modules.module.TestStream()
    ...
    for dbname in dbnames:
        try:
            ...
            if test_file:
                ...
                with dotop.api.Environment.manage():
...
def start(preload=None, stop=False):
    """ Start the dotop http server and cron processor.
    """
    global server
    load_server_wide_modules()
    if dotop.evented:
        server = GeventServer(dotop.service.wsgi_server.application)
    elif config['workers']:
        server = PreforkServer(dotop.service.wsgi_server.application)
    else:
        server = ThreadedServer(dotop.service.wsgi_server.application)
    ...
    if getattr(dotop, 'phoenix', False):
        if watcher:
            watcher.stop()
        _reexec()
