...
def exp_render_report(db, uid, object, ids, datas=None, context=None):
    ...
    cr = dotop.registry(db).cursor()
    try:
        result, format = dotop.report.render_report(cr, uid, ids, object, datas, context)
        if not result:
            tb = sys.exc_info()
            self_reports[id]['exception'] = dotop.exceptions.DeferredException('RML is not available at specified location or not enough data to print!', tb)
        ...
        if hasattr(exception, 'name') and hasattr(exception, 'value'):
            self_reports[id]['exception'] = dotop.exceptions.DeferredException(tools.ustr(exception.name), tools.ustr(exception.value))
        else:
            tb = sys.exc_info()
            self_reports[id]['exception'] = dotop.exceptions.DeferredException(tools.exception_to_unicode(exception), tb)
        self_reports[id]['state'] = True
    cr.commit()
    cr.close()
