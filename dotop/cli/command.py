...
import dotop
from dotop.modules import get_modules, get_module_path
...
class Command(object):
    """Subclass this class to define new dotop subcommands """
...
def main():
    ...
    if len(args) > 1 and args[0].startswith('--addons-path=') and not args[1].startswith("-"):
        ...
        dotop.tools.config._parse_config([args[0]])
        args = args[1:]
    ...
    if len(args) and not args[0].startswith("-"):
        ...
        for module in get_modules():
            if isdir(joinpath(get_module_path(module), 'cli')):
                __import__('dotop.addons.' + module)
