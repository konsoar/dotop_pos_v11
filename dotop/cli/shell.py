...
class Shell(Command):
    """Start dotop in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        dotop.cli.server.report_configuration()
        dotop.service.server.start(preload=[], stop=True)
        ...
        with dotop.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = dotop.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = dotop.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
