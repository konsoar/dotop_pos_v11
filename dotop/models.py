...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (dotop) module name can be in the ``dotop.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``dotop.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['dotop', 'addons']:
...
class BaseModel(object):
    """ Base class for dotop models.

    dotop models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _add_magic_fields(self):
        ...
        * $CONCURRENCY_CHECK_FIELD is a computed field with its computing
        ...
        if self._log_access:
            add('create_uid', fields.Many2one('res.users', string=u"创建人", automatic=True))
            add('create_date', fields.Datetime(string=u"创建时间", automatic=True))
            add('write_uid', fields.Many2one('res.users', string=u"最近修改人", automatic=True))
            add('write_date', fields.Datetime(string=u"最近修改时间", automatic=True))
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = dotop.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or dotop.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from dotop.fields import Char, Integer
                        ...
                        if field.required:
                            try:
                                ...
                                msg = "WARNING: unable to set column %s of table %s not null !\n"\
                                    "Try to re-run: dotop-server --update=module\n"\
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~dotop.fields.Integer`,
          :class:`~dotop.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~dotop.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~dotop.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~dotop.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~dotop.fields.Date` and
              :class:`~dotop.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`dotop.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`dotop.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _dotop/models/relationals/format:

          :class:`~dotop.fields.One2many` and
          :class:`~dotop.fields.Many2many` use a special "commands" format to
          ...
          ``(3, id, _)``
              ...
              :class:`~dotop.fields.One2many`. Can not be used in
              :meth:`~.create`.
          ``(4, id, _)``
              adds an existing record of id ``id`` to the set. Can not be
              used on :class:`~dotop.fields.One2many`.
          ``(5, _, _)``
        ...
        if updates:
            ...
            for name in direct:
                ...
                if callable(field.translate):
                    ...
                    #判断src_trans是否是中文
                    if src_trans:
                        if src_trans and self.check_contain_chinese(src_trans):
                            src_trans2=src_trans
                            src_trans=val
                            val=src_trans2
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an dotop domain.
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~dotop.api.Environment`
        """
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted dotop models.

    dotop models are created by inheriting from this class::
