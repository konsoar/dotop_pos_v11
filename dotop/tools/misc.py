...
"""
Miscellaneous tools used by dotop.
"""
...
from dotop.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if dotop.tools.config['db_host']:
        env['PGHOST'] = dotop.tools.config['db_host']
    if dotop.tools.config['db_port']:
        env['PGPORT'] = str(dotop.tools.config['db_port'])
    if dotop.tools.config['db_user']:
        env['PGUSER'] = dotop.tools.config['db_user']
    if dotop.tools.config['db_password']:
        env['PGPASSWORD'] = dotop.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the dotop root, using a subdir folder.
    ...
    """
    import dotop.modules as addons
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the dotop root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by dotop for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = dotop.modules.module.get_resource_path('base', 'res', 'res.lang.csv')
    try:
...
class upload_data_thread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            fp = urllib.urlopen('http://www.dotop.com/scripts/survey.php', args)
            fp.read()
            fp.close()
        except Exception:
            pass
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating dotop domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('dotop.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('dotop.foo.bar'):
            do_suff()
