#dotop.loggers.handlers. -*- coding: utf-8 -*-
...
import logging
import dotop.release as release
import appdirs
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.dotop.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.dotoprc (or to ~/.dotop_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store dotop data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "dotop.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="dotop.http.rpc.request:DEBUG", help='shortcut for --log-handler=dotop.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="dotop.http.rpc.response:DEBUG", help='shortcut for --log-handler=dotop.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="dotop.http:DEBUG", help='shortcut for --log-handler=dotop.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="dotop.sql_db:DEBUG", help='shortcut for --log-handler=dotop.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate dotop to another language."
    ...
    def parse_config(self, args=None):
        ...
        This method initializes dotop.tools.config and dotop.conf (the
        ...
        Typical usage of this method:
            ...
            dotop.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        dotop.netsvc.init_logger()
        dotop.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'dotop-server.conf')
        else:
            rcfilepath = os.path.expanduser('~/.dotoprc')
            old_rcfilepath = os.path.expanduser('~/.dotop_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.dotoprc' and '.dotop_serverrc' in your path. Please keep only one of "\
                "them, preferrably '.dotoprc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('dotop_RC') or os.environ.get('dotop_SERVER') or rcfilepath)
        self.load()
        ...
        dotop.conf.addons_paths = self.options['addons_path'].split(',')
        dotop.conf.server_wide_modules = [
            m.strip() for m in self.options['server_wide_modules'].split(',') if m.strip()
        ]

    def _is_addons_path(self, path):
        from dotop.modules.module import MANIFEST_NAMES
