...
from dotop.tools.misc import file_open, get_iso_codes, SKIPPED_ELEMENT_TYPES
from dotop.tools.osutil import walksymlinks
from dotop import sql_db, SUPERUSER_ID

_logger = logging.getLogger(__name__)

# used to notify web client that these translations should be loaded in the UI
WEB_TRANSLATION_COMMENT = "dotop-web"
    ...
    def _get_lang(self, frame):
        ...
        if not lang:
            ...
            if not lang:
                ...
                if cr and uid:
                    env = dotop.api.Environment(cr, uid, {})
...
def trans_generate(lang, modules, cr):
    ...
    path_list = [(path, True) for path in dotop.modules.module.ad_paths]
        ...
        for root, dummy, files in walksymlinks(path):
            ...
            if fnmatch.fnmatch(root, '*/static/src/xml*'):
                for fname in fnmatch.filter(files, '*.xml'):
                    babel_extract_terms(fname, path, root, 'dotop.tools.translate:babel_extract_qweb',
                                        extra_comments=[WEB_TRANSLATION_COMMENT])
            if not recursive:
...
def trans_load_data(cr, fileobj, fileformat, lang, lang_name=None, verbose=True, module_name=None, context=None):
    ...
    env = dotop.api.Environment(cr, SUPERUSER_ID, context or {})
    Lang = env['res.lang']
    Translation = env['ir.translation']
