...
import dotop
from dotop.loglevels import ustr
...
# Typical form of references is <timestamp-dotop-record_id-model_name@domain>
...
def generate_tracking_message_id(res_id):
    ...
    try:
        ...
        rnd = random.random()
    rndstr = ("%.15f" % rnd)[2:]
    return "<%.15f.%s-dotop-%s@%s>" % (time.time(), rndstr, res_id, socket.gethostname())
...
def email_send(email_from, email_to, subject, body, email_cc=None, email_bcc=None, reply_to=False,
    ...
    :deprecate: since dotop 6.1, please use ir.mail_server.send_email() instead.
    ...
    if not cr:
        ...
        if db_name:
            local_cr = cr = dotop.registry(db_name).cursor()
        else:
            raise Exception("No database cursor found, please pass one explicitly")

    # Send Email
    try:
        mail_server_pool = dotop.registry(cr.dbname)['ir.mail_server']
        res = False
