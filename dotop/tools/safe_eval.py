...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except dotop.exceptions.except_orm:
        raise
    except dotop.exceptions.Warning:
        raise
    except dotop.exceptions.RedirectWarning:
        raise
    except dotop.exceptions.AccessDenied:
        raise
    except dotop.exceptions.AccessError:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except dotop.http.AuthenticationError:
        raise
    except OperationalError:
        # Do not hide PostgreSQL low-level exceptions, to let the auto-replay
        # of serialized transactions work its magic
        raise
    except dotop.exceptions.MissingError:
        raise
    except Exception, e:
        import sys
        exc_info = sys.exc_info()
