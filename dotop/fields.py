...
from dotop.sql_db import LazyCursor
from dotop.tools import float_precision, float_repr, float_round, frozendict, \
                       html_sanitize, human_size, pg_varchar, ustr, OrderedSet
from dotop.tools import DEFAULT_SERVER_DATE_FORMAT as DATE_FORMAT
from dotop.tools import DEFAULT_SERVER_DATETIME_FORMAT as DATETIME_FORMAT
from dotop.tools.translate import html_translate, _
...
class Field(object):
    """ The field descriptor contains the field definition, and manages accesses
        ...
        recordset. The decorator :meth:`dotop.api.depends` must be applied on
        ...
        is extended (see :class:`~dotop.models.Model`), one can also extend
    ...
    def resolve_deps(self, model):
        ...
        result = []
        #f=open('fff.txt','a')
        #f.write("1.1" + str(self) + "*" + str(model) + "*" + str(self.depends) + "\n")
        ...
        for dotnames in self.depends:
            ...
            for i, fname in enumerate(path):
                ...
                #f.write("------1" + str(field) + "*" + str(self) + "*" + str(path[:i]) + "\n")
...
class Monetary(Field):
    ...
    def convert_to_cache(self, value, record, validate=True):
        if validate:
            ...
            # FIXME @rco-dotop: currency may not be already initialized if it is
...
class Binary(Field):
    ...
    def convert_to_column(self, value, record):
        ...
        # the legacy dotop convention is to transfer and store binaries
