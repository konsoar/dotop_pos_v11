...
import dotop.osv.osv
from dotop import models, api
...
class m(models.Model):
    ...
    def generate_except_osv(self):
        ...
        raise dotop.osv.osv.except_osv('title', 'description')

    @api.multi
    def generate_except_orm(self):
        # title is ignored in the new (6.1) exceptions
        raise dotop.exceptions.except_orm('title', 'description')

    @api.multi
    def generate_warning(self):
        raise dotop.exceptions.Warning('description')

    @api.multi
    def generate_redirect_warning(self):
        action = self.env.ref('test_exceptions.action_test_exceptions')
        raise dotop.exceptions.RedirectWarning('description', action.id, 'Go to the redirection')
    ...
    def generate_access_error(self):
        raise dotop.exceptions.AccessError('description')
    ...
    def generate_user_error(self):
        raise dotop.exceptions.UserError('description')

    @api.multi
    def generate_missing_error(self):
        raise dotop.exceptions.MissingError('description')

    @api.multi
    def generate_validation_error(self):
        raise dotop.exceptions.ValidationError('description')
