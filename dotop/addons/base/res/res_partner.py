...
from dotop import api, fields, models, tools, SUPERUSER_ID, _
from dotop.modules import get_module_resource
from dotop.osv.expression import get_unaccent_wrapper
from dotop.exceptions import UserError, ValidationError
from dotop.osv.orm import browse_record
...
def ch_to_py(name):
    v = ''
    if name:
        pys = pinyin(name, style=pypinyin.FIRST_LETTER, heteronym=True)#开启首字母和多音字
        pys = [''.join(x) for x in itertools.product(*pys)]
        for py in pys:
            v += py
    return v
...
class Partner(models.Model, FormatAddress):
    ...
    @api.depends('name')
    def _compute_mnemonic_code(self):
        for partner in self:
            partner.mnemonic_code = ch_to_py(partner.name)
    
    @api.multi
    def _inverse_mnemonic_code(self):
        return True
    
    @api.multi
    def _get_default_country(self):
        country_id = self.env['res.country'].search([('code','=','CN')], limit=1)
        return country_id and country_id.id or False

    @api.multi
    @api.depends('country_id','state_id','city','county_id','street','street2')
    def _address_show(self):
        for partner in self:
            show = ''
            if partner.country_id:
                show += partner.country_id.name + ' '
            if partner.state_id:
                show += partner.state_id.name + ' '
            if partner.city_id:
                show += partner.city_id.name + ' '
            if partner.county_id:
                show += partner.county_id.name + ' '
            if partner.street:
                show += partner.street
            if partner.street2:
                show += partner.street2
            partner.address_show = show
    
    name = fields.Char(index=True, show_report=True)
    full_name = fields.Char(string=u'全称',index=True, show_report=True)
    display_name = fields.Char(compute='_compute_display_name', store=True, index=True, show_report=True)
    ...
    website = fields.Char(help="Website of Partner or Company", show_report=True)
    comment = fields.Text(string='Notes')

    category_id = fields.Many2many('res.partner.category', column1='partner_id',
                                    column2='category_id', string=u"标签", default=_default_category)
    ...
    function = fields.Char(string='Job Position', show_report=True)
    ...
    street = fields.Char(show_report=True)
    street2 = fields.Char()
    ...
    email = fields.Char(show_report=True)
    email_formatted = fields.Char(
        'Formatted Email', compute='_compute_email_formatted',
        help='Format email address "Name <email@domain>"')
    phone = fields.Char(show_report=True)
    fax = fields.Char(show_report=True)
    mobile = fields.Char(show_report=True)
    ...
    qq = fields.Char('QQ', show_report=True)
    weixin = fields.Char(u"微信", show_report=True)
    city_id = fields.Many2one('res.city',u'城市')
    county_id = fields.Many2one('res.county',u'区/县')
    address_show = fields.Char(u'地址', compute="_address_show")
    mnemonic_code = fields.Char(u'助记码', compute="_compute_mnemonic_code", inverse="_inverse_mnemonic_code", store=True)
    ...
    def write(self, vals):
        ...
        if vals.get('name'):
            if not vals.get('full_name'):
                ...
                if not full_name:
                    vals.update({'full_name':vals.get('name')})
        if vals.get('website'):
        ...
        if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
            ...
            query = """SELECT id
                      ...
                      {where} ({email} {operator} {percent}
                           ...
                           OR {mnemonic_code} {operator} {percent})
                    ...
                    """.format(where=where_str,
                               ...
                               percent=unaccent('%s'),
                               mnemonic_code=unaccent('mnemonic_code'))
