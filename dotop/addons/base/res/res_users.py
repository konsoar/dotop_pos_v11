...
from dotop.exceptions import AccessDenied, AccessError, UserError, ValidationError
from dotop.osv import expression
from dotop.service.db import check_super
from dotop.tools import partition
...
class Users(models.Model):
    """ User class. A res.users record models an dotop user and is different
        from an employee.
    ...
    login = fields.Char(required=True, help="Used to log into the system", show_report=True)
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by dotop (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: dotop.exceptions.AccessDenied when old password is wrong
        :raise: dotop.exceptions.UserError when new password is not set or empty
        """
...
class GroupsView(models.Model):
    ...
    def _update_user_groups_view(self):
        ...
        if view and view.exists() and view._name == 'ir.ui.view':
            ...
            #group_no_one权限组,变更为【系统设置】权限组
            group_erp_manager = view.env.ref('base.group_erp_manager')
            xml1, xml2 = [], []
            xml1.append(E.separator(string=_('Application'), colspan="2"))
            for app, kind, gs in self.get_groups_by_application():
                # hide groups in categories 'Hidden' and 'Extra' (except for group_no_one)
                attrs = {}
                if app.xml_id in ('base.module_category_hidden', 'base.module_category_extra', 'base.module_category_usability','base.module_category_administration'):
                    attrs['groups'] = 'base.group_erp_manager'
...
class ChangePasswordUser(models.TransientModel):
    ...
    def change_password_button(self):
        for line in self:
            if not self.env.user._is_admin() and line.user_id._is_admin():
                raise UserError(u'不能修改系统管理员的密码')
