...
import dotop
from dotop import api, fields, models, modules, tools, _
from dotop.exceptions import AccessDenied, UserError
from dotop.tools.parse_version import parse_version
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('AGPL-3', 'Affero GPL-3'),
        ('LGPL-3', 'General license'),
        ('Other OSI approved licence', 'Other OSI Approved Licence'),
        ('OEEL-1', 'dotop Enterprise Edition License v1.0'),
        ('OPL-1', 'dotop Proprietary License v1.0'),
    ...
    def install_from_urls(self, urls):
        ...
        dotop = dotop.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.iteritems():
                ...
                try:
                    _logger.info('Downloading module `%s` from dotop Apps', module_name)
            ...
            for module_name, url in urls.iteritems():
                if module_name == dotop or not url:
                    continue    # dotop is special case, handled below, and no URL means local module
            ...
            if urls.get(dotop):
                ...
                # copy all modules in the SERVER/dotop/addons directory to the new "dotop" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, dotop, 'addons', d)    # XXX 'dotop' subdirectory ?
                ...
                _logger.info('Copy downloaded module `dotop` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, dotop), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.dotop.com/apps')
