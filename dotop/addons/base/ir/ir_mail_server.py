...
from dotop.exceptions import except_orm, UserError
from dotop.tools import html2text, ustr
...
class IrMailServer(models.Model):
    ...
    def connect(self, host, port, user=None, password=None, encryption=False, smtp_debug=False):
        ...
        if encryption == 'ssl':
            if not 'SMTP_SSL' in smtplib.__all__:
                raise UserError(_("Your dotop Server does not support SMTP-over-SSL. You could use STARTTLS instead."
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.catchall.alias" is not set it defaults to "postmaster-dotop".

        If "mail.catchall.domain" is not set, return None.

        '''
        get_param = self.env['ir.config_parameter'].sudo().get_param
        postmaster = get_param('mail.bounce.alias', default='postmaster-dotop')
...
#         smtp_from = message['Return-Path'] or self._get_default_bounce_address() or message['From']
        smtp_from = self._get_default_bounce_address() or message['From']
