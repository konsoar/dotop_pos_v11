...
from dotop import api, fields, models, SUPERUSER_ID, tools,  _
from dotop.exceptions import AccessError, UserError, ValidationError
from dotop.modules.registry import Registry
from dotop.tools.safe_eval import safe_eval
...
class IrModel(models.Model):
    ...
    def unlink(self):
        ...
        # reload is done independently in dotop.modules.loading.
        if not self._context.get(MODULE_UNINSTALL_FLAG):
...
class IrModelFields(models.Model):
    ...
    def write(self, vals):
        ...
        if vals and self:
            ...
            for item in self:
                if item.state != 'manual' and vals.keys() != ['show_report']:
...
class IrModelConstraint(models.Model):
    """
    This model tracks PostgreSQL foreign keys and constraints used by dotop
    models.
    """
...
class IrModelRelation(models.Model):
    """
    This model tracks PostgreSQL tables used to implement dotop many2many
    relations.
    """
...
class IrModelAccess(models.Model):
    ...
    def group_names_with_access(self, model_name, access_mode): 
        ...
        self._cr.execute("""SELECT
                         ...
                         (model_name,))
        #报权限错误时权限组没有翻译，进行修改 2017-08-10 LJP
        a = self._cr.fetchall()
        res = []
        for x in a:
            s = ''
            if x[0]:
                c = self.env['ir.module.category'].sudo().browse(x[0])
                s = s + c.name + '/'
            if x[1]:
                g = self.env['res.groups'].sudo().browse(x[1])
                s = s + g.name
            res.append(s)
        return res
...
class IrModelData(models.Model):
    """Holds external identifier keys for records in the database.
       This has two main uses:
           ...
           * allows tracking the origin of data installed by dotop
