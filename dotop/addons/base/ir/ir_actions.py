...
import dotop
from dotop import api, fields, models, tools, workflow, _
from dotop.exceptions import MissingError, UserError, ValidationError
from dotop.report.report_sxw import report_sxw, report_rml
from dotop.tools.safe_eval import safe_eval, test_python_expr
...
class IrActionsReportXml(models.Model):
    ...
    def _lookup_report(self, name):
        ...
        if 'report.' + name in dotop.report.interface.report_int._reports:
            return dotop.report.interface.report_int._reports['report.' + name]
        ...
        if row['report_type'] in ('qweb-pdf', 'qweb-html'):
            ...
            kwargs = {}
            if row['parser']:
                kwargs['parser'] = getattr(dotop.addons, row['parser'])
...
class IrActionsServer(models.Model):
    ...
    Since dotop 8.0 a button 'Create Menu Action' button is available on the
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': dotop.exceptions.Warning,
            # record
            'record': record,
            'records': records,
