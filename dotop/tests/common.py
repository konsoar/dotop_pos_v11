# -*- coding: utf-8 -*-
"""
The module :mod:`dotop.tests.common` provides unittest test cases and a few
...
import dotop
from dotop import api

_logger = logging.getLogger(__name__)

# The dotop library is supposed already configured.
ADDONS_PATH = dotop.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = dotop.tools.config['xmlrpc_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = dotop.SUPERUSER_ID


def get_db_name():
    db = dotop.tools.config['db_name']
...
class BaseCase(unittest.TestCase):
    """
    Subclass of TestCase for common dotop-specific code.
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~dotop.models.BaseModel`
        """
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        self.registry = dotop.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        self.uid = dotop.SUPERUSER_ID
        #: :class:`~dotop.api.Environment` for the current test case
...
class SingleTransactionCase(BaseCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.registry = dotop.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.uid = dotop.SUPERUSER_ID
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = dotop.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        dotop.http.root.session_store.save(self.session)
    ...
    def authenticate(self, user, password):
        ...
        # dotopSession.authenticate accesses the current request, which we
        ...
        dotop.http.root.session_store.save(session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('dotop.service.http.request.'):
