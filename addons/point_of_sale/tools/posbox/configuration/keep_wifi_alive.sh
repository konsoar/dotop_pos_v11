...
while true ; do
	if [ -z "$(cat <(ifconfig eth0) <(ifconfig wlan0) | grep "inet " | awk -F: '{print $2}' | awk '{print $1}';)" ] ; then
		...
		sudo /home/pi/dotop/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh "${ESSID}" "${PASSWORD}" "" "NO_AP"
