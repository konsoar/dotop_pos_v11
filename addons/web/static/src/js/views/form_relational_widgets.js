dotop.define('web.form_relational', function (require) {
"use strict";
...
var FieldMany2One = common.AbstractField.extend(common.CompletionFieldMixin, common.ReinitializeFieldMixin, {
    ...
    render_editable: function() {
        ...
        var anyoneLoosesFocus2 = function (event) {
        	var keyCode = event.keyCode;
        	if((keyCode == 13) || (keyCode==9 && !event.shiftKey)){
        		if(self.$el.parents("td").find(".v-editor").length>0){
  	        	  if (self.last_search.length == 0) {
  	        		 self.$el.parents('td').nextAll("td:visible:not(.ntclick)")[0].click();
  	        		 return false;
  	              }
          	   }
        	}
        ...
        this.$input.autocomplete({
            ...
            select: function(event, ui) {
                ...
                if (item.id) {
                    ...
                    if(self.$el.parents("td").find(".v-editor").length>0 && !self.$el.parents("td").hasClass("accountStyle")){
                    	self.$el.parents('td').nextAll("td:visible:not(.ntclick)")[0].click();
                    }
                } else if (item.action) {
                    item.action();
 ...
 *  - get_value() return an dotop write command list
 */
...
return {
    ...
    X2ManyList: X2ManyList,
    One2ManyListView: One2ManyListView,
};
