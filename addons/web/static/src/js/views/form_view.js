dotop.define('web.FormView', function (require) {
"use strict";
...
var FormView = View.extend(common.FieldManagerMixin, {
    ...
    start: function() {
        ...
        this.$(".oe_title,.o_group").on('click', function (e) {
            if(self.get("actual_mode") === "view" && self.$buttons && !$(e.target).is('[data-toggle]')) {
                self.$buttons.find(".o_form_button_edit").dotopBounce();
    ...
    /*
     * 2017-08-05   LJP   Add To Save Show Continue Html
     */    
    _process_save_continue_check: function(save_obj) {
    	var self = this;
    	var create_write = false;
    ...
    on_invalid: function() {
        var warnings = _(this.fields).chain()
            ...
            .map(function (f) {
            	if (f.widget == 'one2many_list' && f.get_active_view() && f.viewmanager.active_view.type == 'list'){//2017-07-31 LJP新增
            		var view = f.get_active_view().controller;
	            	if (view.fields_view && view.editable() && ! _.isEmpty(view.records.records)){
	            		var fields = f.get_active_view().controller.editor.form.fields;
	            		var printstr = '';
	            		_.each(fields, function(field){
	            			if (! field.is_valid()) {
	            				printstr += _.str.sprintf('<li>%s</li>', _.escape(f.string + '  ' + field.string));
	            			}	
	                    });
	            		return printstr;
	                } else {
    ...
    record_created: function(r, prepend_on_create) {
        ...
        if (!r) {
            ...
            //dotop.log("The record has been created with id #" + this.datarecord.id);
...
dotop.define('web.FormRenderingEngine', function (require) {
"use strict";
