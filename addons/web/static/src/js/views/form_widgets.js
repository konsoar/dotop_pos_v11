dotop.define('web.form_widgets', function (require) {
"use strict";
    ...
    render_value: function() {
        ...
        if(this.check_timezone()){
            var options = _.extend({
                ...
                title: _t("Timezone Mismatch : The timezone of your browser doesn't match the selected one. The time in dotop is displayed according to your field timezone."),
            });
...
var FieldBinary = common.AbstractField.extend(common.ReinitializeFieldMixin, {
    ...
    on_file_uploaded: function(size, name) {
        if (size === false) {
            ...
            // TODO: use dotop web crashmanager
...
var FieldStatus = common.AbstractField.extend({
    ...
    /*
     * :deprecated: this feature will probably be removed with dotop v8
     */
...
var AceEditor = common.AbstractField.extend(common.ReinitializeFieldMixin, {
    ...
    willStart: function() {
        if (!window.ace && !this.loadJS_def) {
            this.loadJS_def = ajax.loadJS('/web/static/lib/ace/ace.dotop-custom.js').then(function () {
 ...
 * will substitute to the <field> tags as defined in dotop's views.
 */
 ...
 * tags defined in dotop's form views.
 *
