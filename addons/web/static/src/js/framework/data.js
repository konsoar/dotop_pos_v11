dotop.define('web.data', function (require) {
"use strict";
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<dotop.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of dotop records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the dotop model this dataset will manage
     */
    ...
    get_id_index: function(id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // dotop ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // dotop ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, dotop domain
     ...
     * an SQL-like form usable by dotop's ORM.
     *
     ...
     * dotop's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
