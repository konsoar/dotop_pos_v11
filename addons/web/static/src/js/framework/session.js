dotop.define('web.Session', function (require) {
"use strict";
...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function(parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.dotop._modules && window.dotop._modules.slice()) || [];
        this.server = null;
        ...
        this.setup(origin);
        var debug_param = $.deparam($.param.querystring()).devil;
    ...
    on_modules_loaded: function() {
        ...
        for(var j=0; j<this.module_list.length; j++) {
            ...
            dotop[mod] = {};
            // init module mod
            var fct = dotop._dotop[mod];
            if(typeof(fct) === "function") {
                dotop._dotop[mod] = {};
                for (var k in fct) {
                    dotop._dotop[mod][k] = fct[k];
                }
                fct(dotop, dotop._dotop[mod]);
            }
    ...
    session_reload: function () {
        var result = _.extend({}, window.dotop.session_info);
        delete result.session_id;
    ...
    rpc: function(url, params, options) {
        ...
        if (dotop.debug) {
            options.headers["X-Debug-Mode"] = $.deparam($.param.querystring()).devil;
        }
        ...
        return self.check_session_id().then(function() {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-dotop-Session-Id"] = self.session_id || '';
                }
