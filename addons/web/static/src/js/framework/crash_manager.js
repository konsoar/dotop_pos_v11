dotop.define('web.CrashManager', function (require) {
"use strict";
...
var open_contract_page = function () {
    window.open("http://p.qiao.baidu.com/cps/chat?siteId=11130864&amp;userId=24436581");
};
var alert_buttons = [{text: "确定", close: true, classes: 'btn btn-primary'},{text: "在线客服", click: open_contract_page, classes: 'btn btn-default'}];
var CrashManager = core.Class.extend({
    ...
    rpc_error: function(error) {
        ...
        if (error.data.name === "dotop.http.SessionExpiredException" || error.data.name === "werkzeug.exceptions.Forbidden") {
            this.show_warning({type: "会话超时", data: { message: _t("Your dotop session expired. Please refresh the current web page.") }});
            return;
        }
    ...
    show_error: function(error) {
        ...
        new Dialog(this, {
            title: "系统 " + _.str.capitalize(error.type),
            $content: QWeb.render('CrashManager.error', {error: error}),
            buttons: error.buttons || alert_buttons,
        }).open();
    },
...
var RedirectWarningHandler = Dialog.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new Dialog(this, {
            size: 'medium',
            title: "dotop " + (_.str.capitalize(error.type) || "Warning"),
            buttons: [
...
core.crash_registry.add('dotop.exceptions.RedirectWarning', RedirectWarningHandler);
