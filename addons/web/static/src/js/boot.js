...
(function() {
    ...
    var debug = ($.deparam($.param.querystring()).devil !== undefined);

    var dotop = window.dotop = window.dotop || {};
    _.extend(dotop, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: dotop.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = dotop.__DEBUG__.get_dependencies( job.name );
                ...
                var missing = dotop.__DEBUG__.get_missing_jobs();
                var failed = dotop.__DEBUG__.get_failed_jobs();
                ...
                if (dotop.debug && !_.isEmpty(debug_jobs)) log.push('\nDebug:                  ', debug_jobs);

                if (dotop.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            dotop.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
