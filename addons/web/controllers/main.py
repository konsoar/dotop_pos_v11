...
import dotop
import dotop.modules.registry
from dotop.api import call_kw, Environment
from dotop.modules import get_resource_path
from dotop.tools import topological_sort
from dotop.tools.translate import _
from dotop.tools.misc import str2bool, xlwt
from dotop import http
from dotop.http import content_disposition, dispatch_rpc, request, \
                      serialize_exception as _serialize_exception
from dotop.exceptions import AccessError
from dotop.models import check_method_name
import simplejson
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('dotop.addons.web', "views")
...
def ensure_db(redirect='/web/datapool/selector'):
...
def module_installed_bypass_session(dbname):
    try:
        registry = dotop.registry(dbname)
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, dotop.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = dotop.conf.server_wide_modules or ['web']
    serverside = []
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("dotop.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, dotop has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "dotop-web" in x.auto_comments:
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = dotop.SUPERUSER_ID

        values = request.params.copy()
        try:
            values['databases'] = http.db_list()
        except dotop.exceptions.AccessDenied:
...
class WebClient(http.Controller):
    ...
    def translations(self, mods=None, lang=None):
        ...
        messages = request.env['ir.translation'].sudo().search_read([
            ...
            ('comments', 'like', 'dotop-web'), ('value', '!=', False),
            ('value', '!=', '')],
    ...
    def version_info(self):
        return dotop.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-dotop-Session-Id': request.session.sid}
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = dotop.tools.config['admin_passwd'] == 'admin'
        d['list_db'] = dotop.tools.config['list_db']
        d['langs'] = dotop.service.db.exp_list_lang()
        d['countries'] = dotop.service.db.exp_list_countries()
        # databases list
        d['databases'] = []
        try:
    ...
    @http.route('/web/datapool/selector', type='http', auth="none")
    def selector(self, **kw):
        return self._render_template(manage=False)

    @http.route('/web/datapool/manager', type='http', auth="none")
    def manager(self, **kw):
        return self._render_template()

    @http.route('/web/datapool/create', type='http', auth="none", methods=['POST'], csrf=False)
    ...
    @http.route('/web/datapool/duplicate', type='http', auth="none", methods=['POST'], csrf=False)
    def duplicate(self, master_pwd, name, new_name):
        try:
            if not re.match(DBNAME_PATTERN, new_name):
                raise Exception(_('Invalid database name. Only alphanumerical characters, underscore, hyphen and dot are allowed.'))
            dispatch_rpc('db', 'duplicate_database', [master_pwd, name, new_name])
            return http.local_redirect('/web/datapool/manager')
        except Exception, e:
            error = "Database duplication error: %s" % str(e) or repr(e)
            return self._render_template(error=error)

    @http.route('/web/datapool/drop', type='http', auth="none", methods=['POST'], csrf=False)
    ...
    @http.route('/web/datapool/backup', type='http', auth="none", methods=['POST'], csrf=False)
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            dotop.service.db.check_super(master_pwd)
            ...
            dump_stream = dotop.service.db.dump_db(name, None, backup_format)
    ...
    @http.route('/web/datapool/restore', type='http', auth="none", methods=['POST'], csrf=False)
    def restore(self, master_pwd, backup_file, name, copy=False):
        try:
            ...
            error = u"数据库恢复错误: %s" % str(e) or repr(e)
            return self._render_template(error=error)

    @http.route('/web/datapool/change_password', type='http', auth="none", methods=['POST'], csrf=False)
    ...
    @http.route('/web/datapool/list', type='json', auth='none')
    def list(self):
        """
...
class Session(http.Controller):
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        return request.env['ir.http'].session_info()
    @http.route('/web/session/xg', type='http', auth="none")
    def xg(self, db):
        registry=dotop.modules.registry.Registry(db)
        res={}
        with registry.cursor() as cr:
            env = dotop.api.Environment(cr, dotop.SUPERUSER_ID, {})
            vals=env['base.config.settings'].get_default_fcm_credentials()
            res.update({'access_id':vals.get('fcm_project_id','0')})
            res.update({'access_key':vals.get('xg_access_key','')})
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=dotop.SUPERUSER_ID))
...
class Binary(http.Controller):
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas', filename_field='datas_fname', unique=None, filename=None, mimetype=None, download=None, width=0, height=0):
        ...
        if content and (width or height):
            ...
            if height > 500:
                height = 500
            content = dotop.tools.image_resize_image(base64_source=content, size=(width or None, height or None), encoding='base64', filetype='PNG')
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = dotop.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.iteritems(),
            key=lambda field: dotop.tools.ustr(field[1].get('string', '')))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from dotop's export data to whatever the
